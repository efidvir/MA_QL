...
def run_simulation(number_of_iterations,decay_rate, number_of_agents,force_policy_flag,DISCHARGE, GAMMA, ALPHA):
    ...
    for i in range(number_of_iterations):
        ...
        if i % 1000 == 0:
            print('num_agents: ',number_of_agents,'sp.st',BATTERY_SIZE,'X',MAX_SILENT_TIME,'X', MAX_IDLE_TIME, 'step: ', i, '1000 steps AVG mean score: ',np.mean(score[0][-1000:-1]),epsilon[0])
...
hyper_sim_args = make_pool_parameters(sweep_num_of_agents)
...
if __name__ == '__main__':
    ...
    #surface_mat0 = np.vstack(avg_mat, axis=1)
        ...
        X,Y = np.meshgrid(range(int(number_of_iterations/1000)),sweep_num_of_agents)
